...
当前版本 **v1.1.1**，三种打包方式任选（功能完全一致，仅运行时打包方式不同）：
...
- **WinUI 3 设置窗口**（`OsuCursorWin3`）：
  - 外观：主题（跟随系统/亮色/暗色）、窗口不透明度（0.3–1.0，云母/亚克力模式下固定 100%）、背景图片（选择/恢复默认）、背景模糊半径
  - 场景对齐：普通场景与 DC 场景的独立热点偏移调校
  - 侧边栏：紧凑模式、背景随主题变色、圆角、1px 内缩、展开/收起动画（收起即展开动画反向播放，同一曲线）
  - **改完再应用**：所有改动先进入草稿，窗口右下角浮动「取消更改 / 应用」按钮；拖滑杆不再触发整页重建，也不会打断拖动
  - 标题栏最小化/最大化/关闭按钮颜色跟随应用主题（亮色模式下为黑色）
- **云母 / 亚克力背景**：WinUI 3 合成层系统背景，需 Windows 11；启用时窗口不透明度固定 100%，由合成层接管窗口表面
- **光标动画速度**：旋转、缩放、发光与拖拽回弹统一为 1.33× 速度（保持原轨迹形状，仅整体提速）
