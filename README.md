...
- 使用全屏透明、点击穿透的 WPF 窗口绘制光标
- 临时用透明系统光标隐藏 Windows 原生指针
- 支持拖动时旋转、按下时缩放/发光、悬停可点击元素时发光
- 退出后自动恢复原系统光标
- 音效支持

实际运行时使用的是跟随鼠标的 160px 小窗口，而不是整屏覆盖层；置顶在鼠标移动、点击、Win 键按下或系统光标状态变化时更新，避免开始菜单等非鼠标触发的顶层窗口覆盖光标。
...
程序使用低层鼠标钩子接收移动和点击事件，再由 WPF 按需渲染；没有持续强制全帧刷新，避免光标帧率下降。
