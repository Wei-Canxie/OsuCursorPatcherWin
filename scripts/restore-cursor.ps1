...
Add-Type -Namespace OsuCursorRestore -Name Api -MemberDefinition @'
...
foreach ($id in $ids)
{
    $original = [OsuCursorRestore.Api]::LoadCursor([System.IntPtr]::Zero, [System.IntPtr]$id)
    if ($original -ne [System.IntPtr]::Zero)
    {
        $copy = [OsuCursorRestore.Api]::CopyIcon($original)
        if ($copy -ne [System.IntPtr]::Zero)
        {
            [OsuCursorRestore.Api]::SetSystemCursor($copy, $id) | Out-Null
        }
    }
}

[OsuCursorRestore.Api]::SystemParametersInfo(0x0057, 0, [System.IntPtr]::Zero, 0x0002) | Out-Null
